...
class DataTransformation:
    ...
    def initiate_data_transformation(self,) -> DataTransformationArtifact:
        try:
            ...
            target_feature_train_df = target_feature_train_df.replace(TargetValueMapping().to_dict()) # type: ignore

            #testing dataframe
            input_feature_test_df = test_df.drop(columns=[TARGET_COLUMN],axis=1)
            target_feature_test_df = test_df[TARGET_COLUMN]
            target_feature_test_df = target_feature_test_df.replace(TargetValueMapping().to_dict()) # type: ignore
